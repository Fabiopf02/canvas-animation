...
function move(key, value) {
  ...
  if (key === 't') document.querySelector('body').style.background = '#000'
...
function createObstacleWithRotation() {
  ...
  const obstacle = {
    ...
    move: function () {
      ...
      if (LEFT) this.centerX += person.speed
      if (RIGHT) this.centerX -= person.speed
    },
  }
...
function createTree() {
  ...
  const tree = {
    ...
    move: function () {
      if (LEFT) this.x += person.speed * 0.3
      if (RIGHT) this.x -= person.speed * 0.3
    },
  }
  trees.push(tree)
}
